/* eslint-disable @typescript-eslint/no-unused-vars -- backup 逐字节拷贝：mock 保留 Obsidian API 全签名，未用参数/导入是 API 面的一部分 */
/* eslint-disable @typescript-eslint/no-redundant-type-constituents -- backup 逐字节拷贝：mock 类型面刻意宽松 */
/* eslint-disable @typescript-eslint/no-empty-object-type -- backup 逐字节拷贝：mock 用空对象类型占位 */
/* eslint-disable @typescript-eslint/no-invalid-void-type -- backup 逐字节拷贝：mock 事件类型用 void 联合 */
/* eslint-disable no-restricted-globals -- 浏览器运行时使用 Web 平台 API（localStorage/navigator），Obsidian 桌面端禁令不适用 */
/* eslint-disable no-control-regex -- backup 逐字节拷贝：mock 刻意匹配控制字符的正则 */
/* eslint-disable obsidianmd/hardcoded-config-path -- backup 逐字节拷贝：mock 检查默认配置目录名 */
/* eslint-disable obsidianmd/platform -- backup 逐字节拷贝：web 端用 navigator 探测平台（无 Obsidian Platform） */
/* eslint-disable @microsoft/sdl/no-inner-html -- backup 逐字节拷贝：mock 刻意用 innerHTML 填充 DOM */
// master 无共享 utils/dom/debounce（backup 独有），本地同源复制。
...
// Web-safe YAML frontmatter helpers. Obsidian's `parseYaml`/`stringifyYaml`
// are imported by the agent's project tool — which the web never executes (the
// web proxies all tool calls to the server). Provided here so the shared
// bundle resolves the import; js-yaml is a close semantic match.
...
export type CloseableComponent = {
  close(): void;
}

export type HoverParent = {
...
export type EditorRangeOrCaret = {
  from: EditorPosition;
  to?: EditorPosition;
}
...
export type EditorSelectionOrCaret = {
  anchor: EditorPosition;
  head?: EditorPosition;
}
...
export class Modal implements CloseableComponent {
  ...
  constructor(app: App) {
    ...
    this._bgEl = createDiv(this.containerEl, 'modal-bg');
    this.modalEl = createDiv(this.containerEl, 'modal');

    const modalHeader = createDiv(this.modalEl, 'modal-header');
    ...
    this.contentEl = createDiv(this.modalEl, 'modal-content');
    this._bgEl.addEventListener('click', () => {
  ...
  open(): void {
    this._closeButtonEl.toggleClass('is-hidden', !this.shouldShowCloseButton);
...
export abstract class MarkdownRenderer extends MarkdownRenderChild implements MarkdownPreviewEvents, HoverParent {
  ...
  static async render(
    ...
    // 先抽离 ```lang 围栏代码块，避免后续内联替换（`、*、换行）破坏代码内容，
    // 再以 <pre><code class="language-lang"> 还原——web 端 HTML/SVG 沙箱预览
    // 依赖这个结构（chatCodeBlockPreview 按 language-html / language-svg 增强）。
    const codeBlocks: Array<{ lang: string; code: string }> = []
    const codeBlockToken = (index: number): string =>
      `\u0000YOLO_CODE_BLOCK_${index}\u0000`
    const withPlaceholders = markdown.replace(
      /```([^\n`]*)\n([\s\S]*?)```/g,
      (_match, rawLang: string, code: string) => {
        const index = codeBlocks.length
        codeBlocks.push({ lang: rawLang.trim(), code })
        return codeBlockToken(index)
      },
    )
    const escapedMarkdown = withPlaceholders.replace(
      /[&<>"']/g,
      (character) =>
        ({
          '&': '&amp;',
    ...
    const withCodeBlocks = html.replace(
      /\u0000YOLO_CODE_BLOCK_(\d+)\u0000/g,
      (_match, rawIndex: string) => {
        const entry = codeBlocks[Number(rawIndex)]
        if (!entry) return ''
        const escapedCode = entry.code.replace(/[&<>"']/g, (character) =>
          ({
            '&': '&amp;',
            '<': '&lt;',
            '>': '&gt;',
        ...
        const langClass = entry.lang
          ? ` class="language-${entry.lang.replace(
              /[^a-zA-Z0-9_-]/g,
              '',
            )}"`
          : ''
    ...
    el.replaceChildren(sanitizeHTMLToDom(withCodeBlocks))
  }
}
...
const yoloMobileUserAgent = /Mobi|Android/i.test(yoloUserAgent);
const yoloIosUserAgent = /iPhone|iPad|iPod/i.test(yoloUserAgent);
const yoloAndroidUserAgent = /Android/i.test(yoloUserAgent);

Platform.isDesktop = !yoloMobileUserAgent;
Platform.isDesktopApp = Platform.isDesktop;
Platform.isMacOS =
  typeof navigator !== 'undefined' ? /Mac/.test(navigator.userAgent) : false;
Platform.isMobile = yoloMobileUserAgent;
Platform.isMobileApp = false;
Platform.isAndroidApp = yoloAndroidUserAgent;
Platform.isIosApp = yoloIosUserAgent;
...
if (typeof window !== 'undefined') {
  ;(
    window as typeof window & {
      __yoloPlatformDebug?: Record<string, unknown>
    }
  ).__yoloPlatformDebug = {
    userAgent: yoloUserAgent,
...
export function sanitizeHTMLToDom(html: string): DocumentFragment {
  ...
  template.content.querySelectorAll<HTMLElement>('*').forEach((element) => {
    for (const attribute of [...element.attributes]) {
      ...
      const isDataImage =
        name === 'src' && /^data:image\/(?:gif|jpeg|jpg|png|webp);/iu.test(value)
      const isSafe =
        isDataImage ||
        !/^(?:javascript|vbscript):/iu.test(value) &&
          !/^data:(?:text|application)\//iu.test(value)
      if (!isSafe) {
        element.setAttribute(name === 'href' ? 'href' : attribute.name, name === 'href' ? '#' : '')
      }
    }
  })
...
export const apiVersion: string = '1.8.4';

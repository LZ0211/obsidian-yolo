...
describe('getProtectedVaultPathRules', () => {
  it('covers plugin-private baseDir data, the sync pointer, and the project zone', () => {
    ...
    expect(namePrefixRules).toEqual(
      expect.arrayContaining([{ kind: 'namePrefix', dir: 'YOLO', name: '.yolo_vector_db' }]),
    )
  })

  it('derives the deny set from current baseDir and projectsDir settings', () => {
    const rules = getProtectedVaultPathRules(
      customSettings({ baseDir: 'Config/YoloData', projectsDir: 'Work/Projects' }),
    )
    const paths = rules
      .filter(
...
describe('isProtectedVaultPath', () => {
  ...
  it('denies plugin-private baseDir data', () => {
    ...
    expect(isProtectedVaultPath('YOLO/.yolo_json_db/chats/chat.json', rules)).toBe(
      true,
    )
    ...
    expect(isProtectedVaultPath('YOLO/scheduled-tasks.sqlite', rules)).toBe(true)
    expect(isProtectedVaultPath('YOLO/memory/index.sqlite', rules)).toBe(true)
    expect(isProtectedVaultPath('YOLO/.yolo_vector_db.tar.gz', rules)).toBe(true)
  })
  ...
  it('allows user content including skills, snippets, and learning', () => {
    expect(isProtectedVaultPath('YOLO/skills/review/SKILL.md', rules)).toBe(false)
  ...
  it('respects a custom projectsDir', () => {
    ...
    expect(isProtectedVaultPath('Work/Projects/proj-x/project.md', customRules)).toBe(
      true,
    )
    expect(isProtectedVaultPath('Projects/proj-x/project.md', customRules)).toBe(
      false,
    )
  })
})

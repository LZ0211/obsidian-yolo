...
describe('bash isAvailable (D6b: bash-engine gate unified)', () => {
  ...
  it('every other tool stays in the catalog regardless of bash-engine state (only bash is gated here)', () => {
    ...
    expect(namesWithBashOff).toHaveLength(21)
  })
})
...
describe('bash workspace scope — filesystem-layer enforcement threads through identically (master.md §5)', () => {
  it('createVaultBashFileSystem and createVaultBashSearch receive identical arguments from the old switch case and the new execute()', async () => {
    ...
    expect(fsSpy.mock.calls[0]).toEqual([
      app,
    ...
    expect(oldSearchArgs).toMatchObject({
      app,
      settings,
      workspaceScope: expectedWorkspaceScope,
    })
  })
})

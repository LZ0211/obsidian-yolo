...
export const jsEvalDefinition = defineTool({
  ...
  // calls the Worker makes back to the host. The canonical
  // `workspaceAccessPolicy` and `allowedSkillPaths` are threaded through to
  // `buildJsSandboxProxyHandlers` below so those proxy handlers (in
  // `jsSandboxTool.ts`) can enforce it per-call, the same way `fs_read`'s
  // per-resolved-path check does for wikilinks (issue #577 follow-up:
  // `$vault.readText`/`readBinary`/`list` used to only guard the YOLO
  // user-data root, not workspace scope at all).

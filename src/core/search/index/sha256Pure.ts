...
export function sha256HexPure(value: string): string {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      ...
      schedule[index] =
        (smallSigmaOne(second) +
          schedule[index - 7] +
          smallSigmaZero(first) +
          schedule[index - 16]) >>>
        0
    }
    ...
    for (let index = 0; index < 64; index += 1) {
      const temporaryOne =
        (h +
          ...
          schedule[index]) >>>
        0
      const temporaryTwo = (bigSigmaZero(a) + majority(a, b, c)) >>> 0
      h = g
      g = f
      f = e
      e = (d + temporaryOne) >>> 0

...
describe('deriveMcpSharingCapability', () => {
  ...
  it('declares Hermes supported when its ACP session projection is wired', () => {
  ...
  it('keeps pi unsupported without an MCP process projection', () => {
    const capability = deriveMcpSharingCapability(servers, 'pi', {
      processInjectionWired: true,
    })
    expect(capability.supported).toBe(false)
  })

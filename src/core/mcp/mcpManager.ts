  ...
  private handleUnexpectedServerClose(name: string, client: McpClient): void {
    ...
    void (async () => {
      try {
        ...
        // Settings may have toggled this server off mid-reconnect — or the
        // user may have edited the server config (URL/command/args/auth)
        // while the reconnect was in flight. handleSettingsUpdate aborts this
        // attempt in that flow, but the settings reference can change through
        // other paths, so re-verify the captured snapshot is still what the
        // current settings describe. If it is not, `reconnected` was computed
        // against a stale config — writing it back would clobber the user's
        // edit, so discard it and let the settings-update path own the probe
        // of the new config.

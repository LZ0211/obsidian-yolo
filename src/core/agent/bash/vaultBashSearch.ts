    ...
    try {
      outcome = await runVaultSearchStructured({
        ...
        args: {
          query,
          path: scopePath,
          // Scope filtering after retrieval can outrank in-scope hits below
          // the caller's limit; ask for the service cap and slice down.
          maxResults:
            workspaceAccessPolicy?.enabled === true
              ? SEARCH_MAX_REQUEST
              : maxResults,
          mode: 'hybrid',
        },
